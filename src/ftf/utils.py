...
from ftf.output import Color, Output, TermFeatures
...
def load_toml_file(file_path: Path) -> dict[str, JSONVal]:
    """Load a TOML file.
...
def path_to_data_file(name: str) -> Path:
    ...
    """
    data_dir = Path(str(importlib.resources.files("ftf").joinpath("data")))
...
def get_commit_msg(config: Config, commit_msg: str) -> tuple[str, Path | None]:
    ...
    Returns:
        The commit message and the path to the commit message file
    """
...
def xdg_cache_home() -> Path:
    """Provide a path to the XDG_CACHE_HOME.

    Returns:
        A path the cache
    """
    env = os.environ.get("XDG_CACHE_HOME")
    cache_dir = Path(env) / "ftf" if env else Path.home() / ".cache" / "ftf"
